...
DELIMITER $$
CREATE PROCEDURE RegistraUtente(IN _id INT, _nome VARCHAR(50), _cognome VARCHAR(50), _passw VARCHAR(16), _email VARCHAR(100), _critico BOOL)
BEGIN
	INSERT INTO Utente (ID, Nome, Cognome, Passw, Email, Critico, NomeAbbonamento, Scadenza, ContenutiScaricati)
    VALUES (_id, _nome, _cognome, _passw, _email, _critico, NULL, NULL, NULL);
END $$
...
BEGIN
    ...
    IF _idformatoaudio = 2 AND _idformatovideo = 1
    THEN
		INSERT INTO Offerta (NomeAbbonamento, ID_Contenuto, Visualizzabile)
		VALUES 
            ('premium', max_id_contenuto + 1, FALSE),
    ...
    IF _idformatoaudio = 3 AND _idformatovideo = 2
    THEN
		INSERT INTO Offerta (NomeAbbonamento, ID_Contenuto, Visualizzabile)
		VALUES 
            ('pro', max_id_contenuto + 1, FALSE),
    ...
    IF _idformatoaudio = 4 AND _idformatovideo = 3
    THEN
		INSERT INTO Offerta (NomeAbbonamento, ID_Contenuto, Visualizzabile)
		VALUES 
            ('ultimate', max_id_contenuto + 1, TRUE);
...
BEGIN
    ...
    VALUES (_idutente, _idfilm, _punteggio);
...
BEGIN
    ...
    IF (
		SELECT COUNT(*)
        FROM Download D
		WHERE D.ID_Utente = _idutente AND D.ID_Contenuto = _idocontenuto ) <> 0
	THEN
		SIGNAL SQLSTATE '45000'
